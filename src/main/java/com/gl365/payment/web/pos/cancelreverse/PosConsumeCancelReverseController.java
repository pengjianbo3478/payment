package com.gl365.payment.web.pos.cancelreverse;
 ...
 */
@RestController
public class PosConsumeCancelReverseController extends AbstractBaseController<RollbackReqDTO, RollbackRespDTO> {
	private static final Logger LOG = LoggerFactory.getLogger(PosConsumeCancelReverseController.class);
	@Autowired
	private PosConsumeCancelReverseService posConsumeCancelReverseService;

	@Override
	@PostMapping(value = "posConsumeCancelReverse")
	public RollbackRespDTO service(@RequestBody RollbackReqDTO reqDTO) {
		RollbackRespDTO result = new RollbackRespDTO();
		try {
			result = this.posConsumeCancelReverseService.execute(reqDTO);
