...
@Service
public class OnlineConsumeDBService extends TranDbService {
	...
	public boolean firstCommit(SuperContext bc) throws ServiceException {
		...
		// 写付款表
		List<PayDetail> buildPayDetails = bc.getPayDetails();
		for (PayDetail pd : buildPayDetails) {
			this.payDetailMapper.insert(pd);
		}
		return true;
	}
