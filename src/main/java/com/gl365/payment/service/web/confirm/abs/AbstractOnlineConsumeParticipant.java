...
public abstract class AbstractOnlineConsumeParticipant extends AbstractConsume {
	@Override
	public boolean checkAndSaveInputParameter(BaseContext<OnlineConsumeReqDTO, OnlineConsumeRespDTO> bc) throws InvalidRequestException {
		...
		boolean e = !checkSplitFlag(bc, request.getSplitFlag()); 
		if (b || c || c2 || d || e) return false;
	...
	public PayMain buildPayMain(BaseContext<OnlineConsumeReqDTO, OnlineConsumeRespDTO> bc, DealStatus dealStatus) {
		...
		payMain = payAdapter.setPayMain(payMain, bc.getGl365Merchant(), bc.getGroupMerchant(), getDcType(cardType), Scene.FAST_PAY);
		payMain = payAdapter.setPayMain(payMain, bc.getFinance());
		payMain.setGroupGiftAmount(bc.getFinance().getGiftAmount());
		...
		payMain.setGroupMainuserPay(BigDecimal.ZERO);
		payMain.setGroupPtPay(BigDecimal.ZERO);
		payMain.setGroupMainuserPayBean(BigDecimal.ZERO);
	...
	public void query(BaseContext<OnlineConsumeReqDTO, OnlineConsumeRespDTO> bc) throws ServiceException {
		...
		// 查询群支付商户
		bc.setMerchantNo(bc.getRequest().getGroupMerchantNo());
		bc.setGroupMerchant(this.remoteMicroServiceAgent.queryMerchantInfo(bc));
		bc.setMerchantNo(bc.getRequest().getOrganMerchantNo());
		...
		BigDecimal saleRate = bc.getGroupMerchant().getSaleRate();
