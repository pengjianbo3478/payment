...
import com.gl365.payment.service.web.confirm.abs.AbstractOnlineConsumeInitiator;
import com.gl365.payment.util.BigDecimalUtil;
import com.gl365.payment.util.CalculationFormula;
@Service("onlineConsumeInitiatorService")
public class OnlineConsumeInitiatorServiceImpl extends AbstractOnlineConsumeInitiator implements OnlineConsumeService {
	private static final Logger LOG = LoggerFactory.getLogger(OnlineConsumeInitiatorServiceImpl.class);
	@Autowired
	...
	public boolean bizCheck(BaseContext<OnlineConsumeReqDTO, OnlineConsumeRespDTO> bc) throws ServiceException {
		...
		LOG.info("已支付金额={}", alreadyPay);
		LOG.info("计算发起人应支付={}", request.getTotalAmount().subtract(alreadyPay));
		boolean c = request.getGroupMainUserPay().add(alreadyPay).compareTo(request.getTotalAmount()) == 0;
		if (!c) {
			setReturnResponse(bc, Msg.REQ_0051);
			return false;
		}
		return true;
	}
	...
	public boolean firstCommit(BaseContext<OnlineConsumeReqDTO, OnlineConsumeRespDTO> bc) throws ServiceException {
		...
		OnlineConsumeReqDTO request = bc.getRequest();
		BigDecimal cashAmt = request.getGroupMainUserPay();
		...
		LOG.debug("发起者支付， 乐豆={},银行扣款={}", beanAmount, cashAmount);
		boolean cal = calculation(bc);
		if (!cal) { return false; }
		// 写交易主表
		buildPayMain(bc, DealStatus.WAITING_FOR_PAYMENT);
		// 计算发起者
		calculateGroup(bc, beanAmount);
		// 写付款表
		buildPayDetails(bc);
	...
	private boolean calculation(BaseContext<OnlineConsumeReqDTO, OnlineConsumeRespDTO> bc) {
		...
		// 商户实得金额
		BigDecimal beanAmount = totalAmount.subtract(cashAmount);
	...
	private void calculateGroup(BaseContext<OnlineConsumeReqDTO, OnlineConsumeRespDTO> bc, BigDecimal groupMainuserPayBean) {
		Finance finance = bc.getFinance();
		BigDecimal alreadyGiftAmount = BigDecimal.ZERO;
		for (PayMain pm : ps) {
			alreadyGiftAmount = alreadyGiftAmount.add(pm.getGroupGiftAmount());
		}
		BigDecimal groupGiftAmount = finance.getGiftAmount().subtract(alreadyGiftAmount);
		PayMain pm = bc.getPayMain();
		pm.setGroupGiftAmount(groupGiftAmount);
		pm.setGroupMainuserPayBean(groupMainuserPayBean);
	}
