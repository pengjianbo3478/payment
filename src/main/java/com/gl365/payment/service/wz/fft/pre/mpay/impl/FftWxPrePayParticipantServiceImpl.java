package com.gl365.payment.service.wz.fft.pre.mpay.impl;
...
import com.gl365.payment.service.wz.fft.pre.mpay.abs.AbstractFftWxPrePayParticipant;
import com.gl365.payment.service.wz.rm.pre.pay.RmWxPrePayContext;
import com.gl365.payment.util.Gl365StrUtils;
import com.gl365.payment.util.gson.GsonUtils;
@Service
public class FftWxPrePayParticipantServiceImpl extends AbstractFftWxPrePayParticipant implements FftWxPrePayParticipantService {
	private static final Logger LOG = LoggerFactory.getLogger(FftWxPrePayParticipantServiceImpl.class);
	...
	private void checkMerchantInfo(RmWxPrePayContext ctx) {
		...
		String groupMerchantNo = wxPrePayReqDTO.getGroupMerchantNo();
		...
		Gl365Merchant groupMerchant = this.gl365MerchantService.queryGl365Merchant(groupMerchantNo, organCode, payChannleType);
		ctx.setGl365Merchant(gl365Merchant);
		ctx.setGroupMerchant(groupMerchant);
		this.gl365MerchantService.checkGl365Merchant(gl365Merchant);
		this.gl365MerchantService.checkGl365Merchant(groupMerchant);
	}

	private void queryAccountBalanceInfo(RmWxPrePayContext ctx) {
		LOG.info(this.totalStep() + "-6-效验发起者支付金额");
		String userId = ctx.getRequest().getUserId();
