package com.gl365.payment.service.wz.rm.pre.mpay.abs;
...
public abstract class AbstractRmWxPrePayInitiator extends AbstractPay {
	...
	public void checkRequest(RmWxPrePayContext ctx) {
		...
		this.checkPayRequestService.checkGroupMainuserPay(request.getGroupMainuserPay());
		this.checkPayRequestService.checkScene(request.getScene());
	}
	
	public void checkAmount(RmWxPrePayContext ctx) {
		WxPrePayReqDTO request = ctx.getRequest();
		ps = payMainService.queryByGroupOrderId(request.getGroupOrderId());
		BigDecimal alreadyPay = BigDecimal.ZERO;
		for (PayMain pm : ps) {
			alreadyPay = alreadyPay.add(pm.getTotalAmount());
		}
		boolean c = request.getGroupMainuserPay().add(alreadyPay).compareTo(request.getTotalAmount()) == 0;
		if (!c) throw new ServiceException(Msg.REQ_0051.getCode(),Msg.REQ_0051.getDesc());
	}
	...
	public void putRequest(RmWxPrePayContext ctx) {
		...
		pp.setGroupMainuserPay(request.getGroupMainuserPay());
	}
	...
	public void calculateBeanAmount(RmWxPrePayContext ctx) {
		...
		BigDecimal mainPayAmt = pm.getGroupMainuserPay();
		Map<String, BigDecimal> map = this.wxPrePayCalcuateService.getPayBeanAmt(gl365Merchant, gl365User, mainPayAmt, bAmt);
		BigDecimal cashAmt = map.get(PayCurrency.CASH.getCode());
		BigDecimal beanAmt = pm.getTotalAmount().subtract(cashAmt);
		pm.setGroupMainuserPayBean(mainPayAmt.subtract(cashAmt));
		pm.setBeanAmount(beanAmt);
		pm.setCashAmount(cashAmt);
		BigDecimal alreadyGiftAmount = BigDecimal.ZERO;
		for (PayMain p : ps) {
			alreadyGiftAmount = alreadyGiftAmount.add(p.getGroupGiftAmount());
		}
		BigDecimal groupGiftAmount = pm.getGiftAmount().subtract(alreadyGiftAmount);
		pm.setGroupGiftAmount(groupGiftAmount);
		pm.setGroupPtPay(pm.getTotalAmount().subtract(mainPayAmt));
	}
	...
	public void buildResp(RmWxPrePayContext ctx) {
		...
		data.setGiftAmount(ctx.getPayMain().getGroupGiftAmount());
		data.setPayId(ctx.getPayMain().getPayId());
		data.setCashAmount(ctx.getPayMain().getCashAmount());
		data.setDecAmount(ctx.getPayMain().getDecAmount());
		data.setCreateTime(ctx.getPayMain().getCreateTime());
		ctx.setResponse(resp);
	}

	public void calculateCurrentPayProfit(RmWxPrePayContext ctx) {
		CurrentPayProfitReqDTO request = new CurrentPayProfitReqDTO();
		BeanUtils.copyProperties(ctx.getPayMain(), request);
		List<WxPrePayRespDetailDTO> details = this.gl365SettlementService.calculateCurrentPayProfit(request);
		ctx.getResponse().getData().setDetails(details);
	}
